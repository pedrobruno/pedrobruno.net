...
$sidebar_inner.data('locked',1).css('position','fixed').css('top',-1*x);}}).on('resize.sidebar-lock',function(){wh=$window.height();sh=$sidebar_inner.outerHeight()+30;$window.trigger('scroll.sidebar-lock');}).trigger('resize.sidebar-lock');});var $menu=$('#menu'),$menu_openers=$menu.children('ul').find('.opener');$menu_openers.each(function(){var $this=$(this);$this.on('click',function(event){event.preventDefault();$menu_openers.not($this).removeClass('active');$this.toggleClass('active');$window.triggerHandler('resize.sidebar-lock');});});})(jQuery);(function(){let posts=[];let currentIndex=7;const increment=6;const container=document.querySelector('.posts');const button=document.getElementById('load-more-btn');const containerDiv=document.querySelector('.load-more-container');if(!button||!container)return;fetch('content.json').then(response=>response.json()).then(data=>{posts=data;if(posts.length<=currentIndex){if(containerDiv)containerDiv.style.display='none';}}).catch(err=>{console.error('Error loading content.json:',err);if(containerDiv)containerDiv.style.display='none';});button.addEventListener('click',function(e){e.preventDefault();const nextSet=posts.slice(currentIndex,currentIndex+increment);nextSet.forEach(p=>{const article=document.createElement('article');const cleanUrl=p.url.replace(/index\.html$/,'');article.innerHTML=`<a href="${cleanUrl}"class="image"><img src="${p.image}"alt=""/></a><h3>${p.title}</h3><p>${p.summary}</p><ul class="actions"><li><a href="${cleanUrl}"class="button">More</a></li></ul>`;container.appendChild(article);});currentIndex+=increment;if(currentIndex>=posts.length){if(containerDiv)containerDiv.style.display='none';}});})();(function($){var $window=$(window),$body=$('body'),$searchInput=$('#search-input');if(!$searchInput.length)return;$searchInput.on('keypress',function(e){if(e.which==13){var query=$(this).val().toLowerCase().trim();if(query.length>=3){performSearch(query);}}});function performSearch(query){var script=document.querySelector('script[src*="jquery.min.js"]');var prefix=script?script.getAttribute('src').replace('assets/js/jquery.min.js',''):'';var jsonPath=prefix+'content.json';fetch(jsonPath).then(response=>response.json()).then(data=>{var results=data.filter(item=>{return item.title.toLowerCase().includes(query)||item.summary.toLowerCase().includes(query)||(item.subtitle&&item.subtitle.toLowerCase().includes(query));});renderResults(results,query,prefix);}).catch(err=>console.error('Search error:',err));}
function renderResults(results,query,prefix){var $inner=$('#main .inner');$inner.children().not('#header, #search-bar').remove();var html='<section><header class="major"><h2>Search Results for "'+query+'"</h2></header>';if(results.length===0){html+='<p>No results found.</p>';}else{html+='<div class="posts">';results.forEach(item=>{var url=prefix+item.url.replace(/index\.html$/,'');var imgUrl=prefix+item.image;html+='<article>';html+='    <a href="'+url+'" class="image"><img src="'+imgUrl+'" alt="" /></a>';html+='    <h3><a href="'+url+'">'+item.title+'</a></h3>';html+='    <p>'+item.summary+'</p>';html+='    <ul class="actions"><li><a href="'+url+'" class="button">More</a></li></ul>';html+='</article>';});html+='</div>';}
html+='</section>';$inner.append(html);document.title='Search: '+query+' - Insert Coin';window.scrollTo(0,0);}})(jQuery);