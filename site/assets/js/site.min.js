...
$sidebar_inner.data('locked',1).css('position','fixed').css('top',-1*x);}}).on('resize.sidebar-lock',function(){wh=$window.height();sh=$sidebar_inner.outerHeight()+30;$window.trigger('scroll.sidebar-lock');}).trigger('resize.sidebar-lock');});var $menu=$('#menu'),$menu_openers=$menu.children('ul').find('.opener');$menu_openers.each(function(){var $this=$(this);$this.on('click',function(event){event.preventDefault();$menu_openers.not($this).removeClass('active');$this.toggleClass('active');$window.triggerHandler('resize.sidebar-lock');});});})(jQuery);(function(){let posts=[];let currentIndex=7;const increment=6;const container=document.querySelector('.posts');const button=document.getElementById('load-more-btn');const containerDiv=document.querySelector('.load-more-container');if(!button||!container)return;fetch('content.min.json').then(response=>response.json()).then(data=>{posts=data.posts;if(posts.length<=currentIndex){if(containerDiv)containerDiv.style.display='none';}}).catch(err=>{console.error('Error loading content.min.json:',err);if(containerDiv)containerDiv.style.display='none';});button.addEventListener('click',function(e){e.preventDefault();const nextSet=posts.slice(currentIndex,currentIndex+increment);nextSet.forEach(p=>{const article=document.createElement('article');const cleanUrl=p.url.replace(/index\.html$/,'');article.innerHTML=`<a href="${cleanUrl}"class="image"><img src="${p.image}"alt=""/></a><h3>${p.title}</h3><p>${p.summary}</p><ul class="actions"><li><a href="${cleanUrl}"class="button">More</a></li></ul>`;container.appendChild(article);});currentIndex+=increment;if(currentIndex>=posts.length){if(containerDiv)containerDiv.style.display='none';}});})();(function($){var $window=$(window),$body=$('body'),$searchInput=$('#search-input');if(!$searchInput.length)return;window.SiteSearch={getPrefix:function(){var script=document.querySelector('script[src*="jquery.min.js"]');return script?script.getAttribute('src').replace('assets/js/jquery.min.js',''):'';},performSearch:function(query){var prefix=this.getPrefix();var jsonPath=prefix+'content.min.json';fetch(jsonPath).then(response=>response.json()).then(data=>{var results=data.posts.filter(item=>{return item.title.toLowerCase().includes(query)||item.summary.toLowerCase().includes(query)||(item.subtitle&&item.subtitle.toLowerCase().includes(query));});this.renderResults(results,'Search Results for "'+query+'"',prefix,'Search: '+query);}).catch(err=>console.error('Search error:',err));},renderResults:function(results,title,prefix,pageTitle){var $inner=$('#main .inner');$inner.children().not('#header, #search-bar').remove();var html='<section><header class="major"><h2>'+title+'</h2></header>';if(results.length===0){html+='<p>No results found.</p>';}else{html+='<div class="posts">';results.forEach(item=>{var url=prefix+item.url.replace(/index\.html$/,'');var imgUrl=prefix+item.image;html+='<article>';html+='    <a href="'+url+'" class="image"><img src="'+imgUrl+'" alt="" /></a>';html+='    <h3><a href="'+url+'">'+item.title+'</a></h3>';html+='    <p>'+item.summary+'</p>';html+='    <ul class="actions"><li><a href="'+url+'" class="button">More</a></li></ul>';html+='</article>';});html+='</div>';}
html+='</section>';$inner.append(html);if(pageTitle){document.title=pageTitle+' - Insert Coin';}
window.scrollTo(0,0);}};$searchInput.on('keypress',function(e){if(e.which==13){var query=$(this).val().toLowerCase().trim();if(query.length>=3){window.SiteSearch.performSearch(query);}}});})(jQuery);(function($){const COOKIE_NAME='selected_categories';function setCookie(name,value,days){let expires="";if(days){let date=new Date();date.setTime(date.getTime()+(days*24*60*60*1000));expires="; expires="+date.toUTCString();}
...
return null;}
function getSelectedCategories(){let cookie=getCookie(COOKIE_NAME);return cookie?cookie.split(','):[];}
function toggleCategory(cat){let selected=getSelectedCategories();let index=selected.indexOf(cat);if(index>-1){selected.splice(index,1);}else{selected.push(cat);}
selected=selected.filter(s=>s!=="");setCookie(COOKIE_NAME,selected.join(','),365);return selected;}
function updateUI(){let selected=getSelectedCategories();$('#menu a[data-category]').each(function(){let cat=$(this).data('category');if(selected.indexOf(cat)>-1){$(this).addClass('selected-category');}else{$(this).removeClass('selected-category');}});}
function performCategorySearch(){let selectedSlugs=getSelectedCategories();if(selectedSlugs.length===0){alert("Please select at least one category.");return;}
if(!window.SiteSearch){console.error("SiteSearch not found.");return;}
let prefix=window.SiteSearch.getPrefix();let jsonPath=prefix+'content.min.json';fetch(jsonPath).then(response=>response.json()).then(data=>{let selectedIndices=selectedSlugs.map(slug=>data.categories.indexOf(slug)).filter(idx=>idx!==-1);if(selectedIndices.length===0){window.SiteSearch.renderResults([],'Filtered Results',prefix,'Filtered');return;}
let results=data.posts.filter(post=>{return selectedIndices.every(idx=>post.cats.includes(idx));});let title="Posts in: "+selectedSlugs.join(" + ");window.SiteSearch.renderResults(results,title,prefix,'Category: '+selectedSlugs.join(", "));}).catch(err=>console.error('Category search error:',err));}
$(function(){updateUI();$('#menu').on('click','a[data-category]',function(e){e.preventDefault();let cat=$(this).data('category');toggleCategory(cat);updateUI();});$('#show-categories-btn').on('click',function(e){e.preventDefault();performCategorySearch();});});})(jQuery);